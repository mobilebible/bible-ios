...
- (void)setChapter:(NSUInteger)chapter
{
    ...
    self.navigationItem.title = [NSString stringWithFormat:@"%@ %lu", self.book.shortTitle, (_chapter + 1)];
}
...
- (UIActivityViewController *)activityViewController
{
    ...
    if ([_selectedVerses count] > 0) {
        ...
        for (NSString *verse in chapter.verses) {
            if ([_selectedVerses containsObject:[NSNumber numberWithUnsignedInteger:(i - 1)]]) {
                if (firstVerse) {
                    firstVerse = NO;
                    [textToShare appendFormat:@"%@ %lu : ", _book.shortTitle, (chapter.identifier + 1)];
                }
    ...
    printInfo.jobName = [NSString stringWithFormat:@"%@ %lu", _book.title, (chapter.identifier + 1)];
